...
public abstract class AbstractAlgoBackTest extends SequencerTestCase {
    ...
    protected UnsafeBuffer createTickBackTestSell(){
        final MessageHeaderEncoder headerEncoder = new MessageHeaderEncoder();
        final BookUpdateEncoder encoder = new BookUpdateEncoder();
        ...
        encoder.bidBookCount(3)
                .next().price(104L).size(100L) //bid prices lower than best ask
                .next().price(95L).size(200L)
                .next().price(91L).size(300L);

        encoder.askBookCount(4)
                .next().price(104L).size(100L) //price hits profit target
                .next().price(110L).size(200L)
                .next().price(115L).size(5000L)
                .next().price(119L).size(5600L);
    ...
    protected UnsafeBuffer createTickBackTestStopLoss(){
        final MessageHeaderEncoder headerEncoder = new MessageHeaderEncoder();
        final BookUpdateEncoder encoder = new BookUpdateEncoder();
        ...
        encoder.bidBookCount(3)
                .next().price(80L).size(100L) //major drop in bid prices to simulate bear market and so, stop-loss
                .next().price(75L).size(200L)
